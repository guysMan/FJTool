//
//  FJImageCollectionViewController.m
//  FJTool
//
...
@interface FJImageCollectionViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,UINavigationControllerDelegate, UIImagePickerControllerDelegate>
...
@implementation FJImageCollectionViewController
...
{
    CGFloat width = ([UIScreen mainScreen].bounds.size.width - (kItemsOfRow -1) * kItemsSpace) / kItemsOfRow;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row == self.photoIndex) {
        if (self.selectArray.count >= self.maximumNumberOfSelection) {
            [FJPhotoMgr showAlertWithTitle:[NSString stringWithFormat:@"最多只能选择%lu张图片",(unsigned long)self.maximumNumberOfSelection] message:nil showController:self isSingleAction:YES complete:nil];
            return;
        }
        ...
        if (cell.asset.selected) {
            int count = -1;
            for (FJImageModel *subItem in self.selectArray.mutableCopy) {
                count ++;
            ...
            FJImageModel *item = [FJImageModel new];
